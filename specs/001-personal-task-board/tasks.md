...
- [ ] T002 Install dependencies: React, Tailwind CSS, `@dnd-kit/core`, `@dnd-kit/sortable`, `vitest`, `@testing-library/react`.
...
- [ ] T014 Integrate `@dnd-kit/core` and related `@dnd-kit` utilities behind a small adapter (`src/lib/dnd/*`).
- [ ] T015 Implement drag-and-drop functionality in the `Board` component using the adapter.
- [ ] T016 Update state on drag end / drop and ensure targeted updates (avoid full-list re-renders).
- [ ] T017 Test drag-and-drop interactions for usability and accessibility, including keyboard DnD flows.
...
- [ ] T022 Optimize performance for rendering up to 500 tasks. See **T037** for deterministic verification and CI checks.
...
- [ ] T037 Deterministic performance verification: implement a CI-friendly verification harness that validates performance without relying on absolute millisecond thresholds.
...
- [ ] T038 Add a concrete accessibility checklist entry in `checklists/requirements.md` and connect verification tests (axe smoke test + manual checklist).
 - [ ] T039 Add CI job: run `vitest` and fail on test failures.
 - [ ] T040 Add CI axe smoke test: run `jest-axe` or `axe` checks and fail CI on critical accessibility violations.
