...
Following the constitution's Testing Principles, this project adopts Test-Driven Development (TDD) with a RED-GREEN-REFACTOR cycle. All implementation begins with failing tests, followed by minimal code to pass, then refactoring while maintaining test coverage. The testing pyramid targets 70% unit tests, 20% integration tests, and 10% E2E tests.

### Test Organization
- **Unit Tests**: `tests/unit/` mirroring `src/` structure (e.g., `tests/unit/components/Board.test.tsx`)
- **Integration Tests**: `tests/integration/` grouped by feature (e.g., `tests/integration/tasks/`, `tests/integration/board/`)
- **E2E Tests**: `tests/e2e/` grouped by user journey (e.g., `tests/e2e/create-task.spec.ts`)

### TDD Workflow by Component

#### localStorage Utilities (Phase 2)
- **RED**: Write failing unit tests in `tests/unit/utils/localStorage.test.ts` for save/load operations, error handling, and data validation
- **GREEN**: Implement minimal localStorage functions with TypeScript strict mode
- **REFACTOR**: Extract helpers, add error recovery, maintain 100% coverage

#### Board Component (Phase 2)
- **RED**: Write failing unit tests for task rendering, column organization, and state updates
- **GREEN**: Implement Board with useState/useReducer, ensure tests pass
- **REFACTOR**: Optimize re-renders with React.memo, verify performance tests

#### TaskCard Component (Phase 2)
- **RED**: Write failing tests for display, edit/delete triggers, and accessibility
- **GREEN**: Implement TaskCard with props and event handlers
- **REFACTOR**: Add ARIA attributes, ensure keyboard navigation works

#### Column Component (Phase 2)
- **RED**: Write failing tests for task filtering and DnD interactions
- **GREEN**: Implement Column with filtered task lists
- **REFACTOR**: Optimize with useMemo for derived state

#### TaskForm Component (Phase 3)
- **RED**: Write failing tests for create/edit modes, validation, and submission
- **GREEN**: Implement form with controlled inputs and validation
- **REFACTOR**: Add accessibility features, extract validation helpers

#### State Management (Phase 4)
- **RED**: Write failing integration tests for CREATE/UPDATE/DELETE/MOVE actions
- **GREEN**: Implement useReducer with actions and localStorage persistence
- **REFACTOR**: Add optimistic updates, error boundaries

#### Drag-and-Drop Integration (Phase 3)
- **RED**: Write failing integration tests for task movement between columns
- **GREEN**: Implement @dnd-kit adapter with drop handlers
- **REFACTOR**: Add keyboard accessibility, ARIA attributes

#### Persistence (Phase 4)
- **RED**: Write failing integration tests for hydration and data integrity
- **GREEN**: Wire state to localStorage with error handling
- **REFACTOR**: Add migration logic for future schema changes

### E2E Testing (Phase 5)
- **Critical Journeys**: E2E tests for create-task and manage-board workflows using Playwright
- **Responsive UI**: E2E tests for mobile/tablet/desktop adaptations
- **Accessibility**: Automated axe checks for zero critical violations

### Quality Gates & Tools
- **Coverage**: 80% line, 75% branch, 75% mutation score (Stryker)
- **Tools**: Jest 29.x + ts-jest for unit/integration, Playwright 1.40+ for E2E, Stryker for mutation testing
- **Static Analysis**: TypeScript strict mode + ESLint
- **Commands**: `npm run test:unit`, `npm run test:integration`, `npm run test:e2e`, `npm run test:coverage`, `npm run test:mutation`

### Testing Anti-Patterns to Avoid
- No testing of private methods or internal state
- No interdependent or flaky tests
- No tautological assertions (expect(x).toBe(x))
- No tests without meaningful assertions
- Mock external services only; test owned code directly

All tests follow AAA pattern, use beforeEach for setup, and ensure independence. The full test suite must pass before any merge.
